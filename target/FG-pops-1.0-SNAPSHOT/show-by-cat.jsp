...
<head>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/ajax.js" defer></script>
</head>
...
<main class="app">
    <section class="content grid-y ">
        ...
        <div class="grid-x justify-center">
            ...
            <select onload="ajax()"  id="categoryId">

            </select>
            <button class="btn primary "onclick="show()">Visualizza per categoria</button> </div>

        <div class="grid-x  body  ">
            <%session.setAttribute("session",ac); %>
            <% ProductDao pd= new ProductDao();
                ArrayList<Product> list= (ArrayList<Product>)pd.doRetrieveAll();
                for (Product p: list) {
        ...
        <label style="align-self: flex-end;color: darkred;margin-right: 0.5rem;font-weight: bold;font-size: 1rem;">Benvenuto <%=ac.getFirstname()%> <%=ac.getLastname()%> </label>
